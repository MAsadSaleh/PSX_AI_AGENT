...
try:
    ...
    selected_ticker = st.selectbox("Select a stock ticker:", all_tickers)

    # Get data for the selected company
    st.write(f"\nFetching data for {selected_ticker}...")
    data = stocks(selected_ticker, start=datetime.date(2023, 1, 1), end=datetime.date.today())

    # Display the first few rows of the data
    st.write(f"\nFirst few rows of {selected_ticker} stock data:")
